...
import Main.Data.PerMissionData;
import Main.Main;
...
public class InitUtil {
    ...
    public static void initEvents() throws ClassNotFoundException, IllegalAccessException, InstantiationException, IOException {
        DataUtil.initCategory();
        if (new File("./permission").exists()) {
            Main.staticData.perMissionData = (PerMissionData) DataUtil.readObj(new File("./permission"));

        } else {
            Set<Class<?>> classes = getClasses("Main.Class.Events");
            for (Class theClass : classes
            ) {
                Event event = (Event) theClass.newInstance();
                Main.staticData.perMissionData.events.put(theClass.getSimpleName(), event);
            }
        }
